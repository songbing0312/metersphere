...
public class TestResourcePoolService {
    ...
    public UpdatePoolDTO checkHaveTestUsePool(String poolId) {
        ...
        List<LoadTest> loadTests = microService.getForDataArray(MicroServiceName.PERFORMANCE_TEST, "/performance/get/using/pool/" + poolId, LoadTest.class);
        ...
        // api
        List<String> apiNames = baseTaskMapper.checkActuator(testResourcePool.getId());
        ...
        if (pool != null) {
            ...
            if (pool.getType().equals("NODE")) {
                for (TestResource resource : resources) {
                    Map object = JSON.parseObject(resource.getConfiguration(), Map.class);
